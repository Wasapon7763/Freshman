...
void solve3(){ //２数の候補を見つけて分岐
  ...
  for (i=0;i<9;i++){
    for (j=0;j<9;j++){
      ...
      if (avanum[i][j]==2){
	for (k=0;k<9;k++){
	  printf("%d ",avanumlist[i*9+j][k]);
	  if (a1!=-1&&avanumlist[i*9+j][k]!=-1) a2=avanumlist[i*9+j][k];
	  if (a1==-1&&avanumlist[i*9+j][k]!=-1) a1=avanumlist[i*9+j][k];
	}
	break;
      }
    }
